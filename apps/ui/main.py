...
def _render_live_feed_tab() -> None:
    ...
    with col_table:
        ...
        if events:
            ...
            st.info(
                "No enriched events available yet. Produce some demo events to get "
                "started."
            )
...
def _render_monitoring_tab() -> None:
    ...
    with col1:
        ...
        if metrics_summary:
            ...
            st.metric(
                "Events processed to DLQ",
                metrics_summary.get("events_processed_dlq", 0),
            )
